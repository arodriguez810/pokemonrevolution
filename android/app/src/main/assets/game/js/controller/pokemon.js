...
function RUNBATTLE($scope, $timeout) {
    $scope.willreturn = undefined;
    $scope.POKEMON = {
        EVOL: 22,
        QUALITY: 35,
        QUALITY_ELE: 100,
    ...
    $scope.POKEMON.categoriesarray = [];
    for (var category of $scope.POKEMON.categories) {
        $scope.POKEMON.categoriesarray.push(category.name);
    }
    $scope.POKEMONAPI = {
    ...
    $scope.STATGLOBAL = ["atk", "def", "spa", "spd", "spe", "accuracy", "evasion"];
    $scope.TYPEDEFF = [
        "Bug",
        "Dragon",
        "Electric",
    ...
    $scope.POKEMOMFIND = {
        ...
        WASCATCH: function ($scope, info, intento) {
            var pokeball = $scope.POKEMOMFIND.tier_get(SESSION.tier).pokeball;
        ...
        filterFirst: function (filter) {
            var data = $scope.APIS.POKEDEX.filter(filter);
            if (data.length > 0)
                return data[0];
            return undefined;
        },
        filterList: function (filter) {
            return $scope.APIS.POKEDEX.filter(filter);
        },
        firstByName: function (name) {
            return $scope.POKEMOMFIND.filterFirst(d => {
        ...
        QUALITY: function (pokemon) {
            ...
            if (pokemon.elegido) {
                realq = realq > $scope.POKEMON.QUALITY_ELE ? $scope.POKEMON.QUALITY_ELE : realq;
            } else {
                realq = realq > $scope.POKEMON.QUALITY ? $scope.POKEMON.QUALITY : realq;
            }
            return realq;
        },
        EVOLPROB: function (name, kill) {
            var pokemon = $scope.POKEMOMFIND.firstByName(name.toLowerCase());
            var killpoke = $scope.POKEMOMFIND.firstByName((kill || "").toLowerCase());
            if (pokemon) {
                var mega = false;
                ...
                if (evos) {
                    if (evos.length > 0) {
                        var epokemon = $scope.POKEMOMFIND.firstByName(evos[0].toLowerCase());
                        var prob = Math.ceil($scope.POKEMON.EVOL - ((epokemon.power - pokemon.power) / 10) - ((pokemon.power - 100) / 100));
                        if (killpoke) {
        ...
        TRAINER: function (tier, type, base) {
            var trainer = $scope.POKEMON.trainersRange[type || "noob"];
            var pokemons = [];
            var extralevel = 0;
            var rebote = 0;
            for (var pokemon of trainer) {
                var found = false;
                while (!found) {
                    var index = $scope.POKEMON.categoriesarray.indexOf(tier) + pokemon + (rebote || extralevel);
                    if (!$scope.POKEMON.categoriesarray[index])
                        rebote = extralevel - 1;
                    var pokis = $scope.POKEMOMFIND.WILD($scope.POKEMON.categoriesarray[index], base || false, true);
                    if (pokis === null) {
                        extralevel++;
                        if (rebote)
                            rebote--;
        ...
        tier_get: function (tier) {
            var tiers = $scope.POKEMON.categories.filter(d => {
                return d.name === tier;
            });
            if (tiers.length > 0)
                tiers = tiers[0];
            else
                tiers = $scope.POKEMON.categories[0];
            return tiers;
        },
        WILD: function (tier, base, exclusive) {
            var tiers = $scope.POKEMOMFIND.tier_get(tier);
            var potency = getRandomInt(100);

            var next = 0;
            var ranges = $scope.ACTIONS.AMBIENT.TIME() === "Sunset" ? $scope.POKEMON.wildRangesSunset : $scope.POKEMON.wildRanges;
            ...
            if (next > 0)
                tiers = $scope.POKEMOMFIND.next(tiers, next);

            var pokemons = eval(` $scope.APIS.POKEDEX.filter(d=>{ return ${tiers.rules.split("&&")[1]} && d.forme!=='Gmax' })`);
            if (exclusive)
                pokemons = eval(` $scope.APIS.POKEDEX.filter(d=>{ return ${tiers.rules} && d.forme!=='Gmax' })`);
            if (base) {
                if (base.length) {
            ...
            if (pokemons.length > 0) {
                ...
                var generated = $scope.POKEMOMFIND.GENERATE(pokemon.keyname);
        ...
        next: function (tier, count) {
            ...
            var begin = false;
            for (var category of $scope.POKEMON.categories) {
                if (next >= ($scope.POKEMON.categories.length - 1))
                    return category;
        ...
        EVOLUTION: function (pokemon, evol) {
            var newpokemon = $scope.POKEMOMFIND.GENERATE(evol, pokemon.shiny);
        ...
        GENERATE: function (name, shinyexact, elegido) {
            ...
            if (pokemon) {
                ...
                var mymoves = $scope.MOVESFIND.POKEMON_MOVES_GENERATE(name);
                var statis = OSO(pokemon.baseStats);
                statis.accuracy = 100;
                statis.evasion = 0;
                var finalability = $scope.MOVESFIND.ABILITY(ability);
                ...
                var toreturn = {
                    ...
                    giga: giga,
                    battle: OSO($scope.BATTLEDEFAULT),
                    power: pokemon.power
                };
                if (elegido) {
            ...
            if (pokemons.length >= 6) {
                swal({
                    ...
                    if (bobedas.length >= 10) {
                        ...
                        bobedas.push(item);
                        $scope.ACTIONS.GAME.SESSION({bobeda: bobedas});
                ...
                pokemons.push(item);
                $scope.ACTIONS.GAME.SESSION({pokemons: pokemons});
                swal(`${item.name}, ` + LANGUAGE.t(`has been kept in your vault`));
                swal(`${item.name}, ` + LANGUAGE.t(`has joined your team.`));
            }
        },
        ADDWILD: function () {
            var item = $scope.POKEMOMFIND.WILD(SESSION.tier);
            $scope.POKEMOMFIND.ADD(item);
        },
        DELETE: function (item, index) {
            swal({
                ...
                $scope.ACTIONS.GAME.SESSION({pokemons: SESSION.pokemons});
        ...
        TRASLADE: function (item, index) {
            swal({
                ...
                if (bobedas.length >= 10) {
                    ...
                    bobedas.push(item);
                    $scope.ACTIONS.GAME.SESSION({bobeda: bobedas, pokemons: SESSION.pokemons});
                    swal(`${item.name}, ` + LANGUAGE.t(`has been kept in your vault`));
                }
                $scope.$evalAsync();
            });
        ...
        INCLUDE: function (item, index) {
            swal({
                ...
                if (pokemons.length >= 6) {
                    ...
                    pokemons.push(item);
                    $scope.ACTIONS.GAME.SESSION({bobeda: SESSION.bobeda, pokemons: pokemons});
                    swal(`${item.name}, ` + LANGUAGE.t(`has been kept in your vault`));
                    swal(`${item.name}, ` + LANGUAGE.t(`has joined your team`));
                }
                $scope.$evalAsync();
        ...
        DELETE_BOBEDA: function (item, index) {
            swal({
                ...
                SESSION.bobeda.splice(index, 1);
                $scope.ACTIONS.GAME.SESSION({bobeda: SESSION.bobeda});
    ...
    $scope.MOVESFIND = {
        POKEMON_MOVES: function (name) {
            var pokemon = $scope.POKEMOMFIND.firstByName(name);
            if ($scope.APIS.LEARNS[(pokemon.baseSpecies ? pokemon.baseSpecies.toLowerCase() : name)])
                return $scope.APIS.LEARNS[(pokemon.baseSpecies ? pokemon.baseSpecies.toLowerCase() : name)];
            else
                return $scope.APIS.LEARNS[name];
        },
        /**
         * @return {null}
         */
        INFO: function (name) {
            var search = $scope.APIS.MOVES.filter(d => {
        ...
        POKEMON_MOVES_GENERATE: function (name) {
            var pokemon = $scope.POKEMOMFIND.firstByName(name);
            var moves = $scope.MOVESFIND.POKEMON_MOVES_INFO(name);
            var moveCount = 0;
            var random = [];
            var nums = [];
    ...
    $scope.POKEMONBATTLE = {
        ...
        LAUNCH: function ($scope, tier, trainer, win, loose, revenge) {
            ...
            $scope.PKM.pokeballLevel = $scope.POKEMON.categoriesarray.indexOf(tier);
            $scope.PKM.catchMove = {
                "num": 0,
                "accuracy": true,
                "basePower": 0,
            ...
            $scope.ACTIONS.GAME.SCREEN(0.5, "#000", 1, function () {
                ...
                $scope.BATTLEOBJS.FRIENDINDEX = $scope.POKEMONBATTLE.LIVES($scope, $scope.session.pokemons, 99)[0];
                $scope.BATTLEOBJS.TARGETINDEX = 0;
                $scope.BATTLEOBJS.BG = $scope.POKEMONBATTLE.DRAWBG($scope);
                ...
                if ( $scope.CURRENTONLINE) {
                    ...
                    $scope.BATTLEOBJS.trainerProp = $scope.CURRENTTRAINER;
                    $scope.BATTLEOBJS.ENEMY = $scope.POKEMONBATTLE.ENEMY($scope, trainer);
                    var finalTypes = $scope.CURRENTTRAINER.level;
                    if (revenge) {
                    ...
                    $scope.BATTLEOBJS.TARGETS =
                        $scope.BATTLEOBJS.TARGETS = $scope.BATTLEOBJS.trainerProp.predeterminedTeam ||
                            $scope.POKEMOMFIND.TRAINER(tier, finalTypes, $scope.CURRENTTRAINER.types);
                } else {
                    $scope.BATTLEOBJS.ENEMY = $scope.POKEMONBATTLE.ENEMY($scope, trainer);
                    $scope.BATTLEOBJS.TARGETS = [$scope.POKEMOMFIND.WILD(tier)];
                }
                ...
                $scope.ACTIONS.LOAD.ADD(resources, function () {
                    $scope.$evalAsync();
                    ...
                    $scope.ACTIONS.GAME.ALPHABASE(layerBattle, 0.5, 1, function () {
                        ...
                        $timeout(() => {
                                $scope.ACTIONS.CAMERA.ZERO();
                                $scope.BATTLEOBJS.FRIEND = $scope.POKEMONBATTLE.FRIEND($scope);
                                $scope.BATTLEOBJS.HERO.body.gotoAndPlay($scope.POKEMONBATTLE.FORMS.punch);
                                $scope.ACTIONS.ANIMATION.THROWNATURAL('Pokeball_' + $scope.PKM.friend().quality,
                                    $scope.BATTLEOBJS.HERO.body.x,
                                    $scope.BATTLEOBJS.HERO.body.y,
                                    $scope.POKEMONBATTLE.XS($scope.BATTLEOBJS.FRIEND.body.x + ($scope.BATTLEOBJS.FRIEND.body.width / 2)),
                                    $scope.POKEMONBATTLE.YS($scope.BATTLEOBJS.FRIEND.body.y + ($scope.BATTLEOBJS.FRIEND.body.height / 2)),
                                    true,
                                    function () {
                                        $scope.ACTIONS.CAMERA.ZERO();
                                        $scope.ACTIONS.ANIMATION.PLAYNATURAL($scope.PKM.friend().superShiny ? 'SuperShiny' : ($scope.PKM.friend().shiny ? 'Starts' : 'OutPokeball'),
                                            $scope.POKEMONBATTLE.XS($scope.BATTLEOBJS.FRIEND.body.x + ($scope.BATTLEOBJS.FRIEND.body.width / 2)),
                                            $scope.POKEMONBATTLE.YS($scope.BATTLEOBJS.FRIEND.body.y + ($scope.BATTLEOBJS.FRIEND.body.height / 2)),
                                            function () {
                                                ...
                                                if (trainer) {
                                                    $timeout(() => {
                                                            $scope.ACTIONS.CAMERA.ZERO();
                                                            $scope.BATTLEOBJS.TARGET = $scope.POKEMONBATTLE.TARGET($scope);
                                                            $scope.BATTLEOBJS.ENEMY.body.gotoAndPlay($scope.POKEMONBATTLE.FORMS.punch);

                                                            if (!$scope.BATTLEOBJS.isWild)
                                                                $scope.PKM.talk(`${LANGUAGE.t("I choose you")} ${$scope.PKM.target().name}`, 2);
                                                            $scope.ACTIONS.ANIMATION.THROWNATURAL('Pokeball_' + $scope.PKM.target().quality,
                                                                $scope.BATTLEOBJS.ENEMY.body.x,
                                                                $scope.BATTLEOBJS.ENEMY.body.y,
                                                                $scope.POKEMONBATTLE.XS($scope.BATTLEOBJS.TARGET.body.x + ($scope.BATTLEOBJS.TARGET.body.width / 2)),
                                                                $scope.POKEMONBATTLE.YS($scope.BATTLEOBJS.TARGET.body.y + ($scope.BATTLEOBJS.TARGET.body.height / 2)),
                                                                true,
                                                                function () {
                                                                    $scope.ACTIONS.CAMERA.ZERO();
                                                                    $scope.ACTIONS.ANIMATION.PLAYNATURAL($scope.PKM.target().superShiny ? 'SuperShiny' : ($scope.PKM.target().shiny ? 'Starts' : 'OutPokeball'),
                                                                        $scope.POKEMONBATTLE.XS($scope.BATTLEOBJS.TARGET.body.x + ($scope.BATTLEOBJS.TARGET.body.width / 2)),
                                                                        $scope.POKEMONBATTLE.YS($scope.BATTLEOBJS.TARGET.body.y + ($scope.BATTLEOBJS.TARGET.body.height / 2)),
                                                                        function () {
...
//Calculos y animaciones
        CALCS: {
            MULTIPLIER: function (types, moveType, intt) {
                ...
                for (var type of types) {
                    var taken = $scope.APIS.TYPES[type].damageTaken[moveType];
                    if (taken === 1)
                        multiplier++;
                    if (taken === 2)
                        multiplier--;
            ...
            DAMAGE: function ($scope, info, critial, base, last, miss) {
                ...
                if (["judgment", "hiddenpower", "multiattack", "naturepower", "revelationdance", "secretpower", "technoblast"].indexOf(info.move.keyname) !== -1) {
                    info.move.type = randomArray($scope.TYPEDEFF);
                }
                ...
                var multiplier = $scope.POKEMONBATTLE.CALCS.MULTIPLIER(info.pokemonTarget.types, info.move.type);
                ...
                if (info.pokemonTarget.battle.status) {
                    if (info.pokemonTarget.battle.status.indexOf("Am_") !== -1) {
                        ...
                        multiplier = $scope.POKEMONBATTLE.CALCS.MULTIPLIER(typesDD, info.move.type);
                    }
                }
                ...
                if (info.move.num === 251) {
                    ...
                    $scope.ACTIONS.MESSAGE.NOTI(`${info.pokemonFriend.name} ${LANGUAGE.t("ha tomado la fuerza de")} ${prota.name}`);
                }
                ...
                if (info.targetAbility.blocks)
                    if (info.targetAbility.blocks.indexOf('target_calcstats') !== -1) {
                        if (info.targetAbility.code) {
                            eval(info.targetAbility.code);
                            if ($scope.willreturn) {
                                var copyer = OSO($scope.willreturn);
                                $scope.willreturn = undefined;
                                return copyer;
                            }
                ...
                if ($scope.BATTLEOBJS.terrain === "magicroom") {
                    $scope.POKEMONBATTLE.CLEARSTATS(info.pokemonFriend);
                ...
                if (info.pokemonFriend.battle.status) {
                    ...
                    if (info.pokemonFriend.battle.status === "charge") {
                        if (info.move.type === "Electric") {
                            fata = fata * 2;
                            $scope.ACTIONS.MESSAGE.NOTI(`${info.pokemonFriend.name} ${LANGUAGE.t("atacó con sobrecarga")}!`);
                        }
                ...
                if (info.pokemonTarget.battle.status) {
                    ...
                    if (info.pokemonTarget.battle.status === "auroraveil") {
                        $scope.ACTIONS.MESSAGE.NOTI(`${info.pokemonTarget.name} ${LANGUAGE.t("ha reducido daño por auroraveil")}!`);
                        fata = fata / 2;
                    }
                    ...
                    if (info.pokemonTarget.battle.status === "reflect") {
                        if (info.move.num === 280) {
                            ...
                            if (info.move.category === "Physical") {
                                fata = fata / 2;
                                $scope.ACTIONS.MESSAGE.NOTI(`${info.pokemonTarget.name} ${LANGUAGE.t("ha reflejado daño")}!`);
                            }
                        }
                    }
                ...
                if ([374, 373, 365, 450].indexOf(info.move.num) !== -1) {
                    ...
                    for (var stat of stats)
                        if (info.pokemonFriend.battle.stats[stat] < 0)
                            info.pokemonFriend.battle.stats[stat] = temp2[stat];
                    $scope.POKEMONBATTLE.CLEARGOODSTATS(info.pokemonTarget);
                }
                    ...
                    if (info.move.flags.mirror) {
                        if (last) {
                            if (info.move.shortDesc.indexOf("doubles") !== -1) {
                                $scope.ACTIONS.MESSAGE.NOTI(`${info.pokemonFriend.name} ${LANGUAGE.t("aumentó en venganza")}!`);
                                fata = fata * 2;
                            }
                        }
                    }
                ...
                if (info.move.keyname === "shoreup") {
                    ...
                    $scope.ACTIONS.ANIMATION.PLAYNATURAL("Heal",
                        $scope.POKEMONBATTLE.XS(info.friend.body.x + (info.friend.body.width / 2)),
                        $scope.POKEMONBATTLE.YS(info.friend.body.y + (info.friend.body.height / 2)),
                        function () {
                ...
                if (info.move.category !== "Status") {
                    ...
                    if (info.pokemonFriend.types.indexOf(info.move.type) === -1)
                        for (var type of info.pokemonFriend.types) {
                            var taken = $scope.APIS.TYPES[type].damageTaken[info.move.type];
                            if (taken === 1)
                                penaltyType -= 0.25;
                        }
                    ...
                    if (info.move.recoil) {
                        ...
                        $scope.ACTIONS.MESSAGE.NOTI(`${info.pokemonFriend.name} ${LANGUAGE.t("se auto dañó")}!`);
                    }
                } else {
                    damage = 0;
                }
            ...
            VolatileStatus: function (info, $scope, last, miss) {
                ...
                if (miss)
                    return;
                var MeMulti = $scope.POKEMONBATTLE.CALCS.MULTIPLIER(info.pokemonFriend.types, info.move.type) === "=-1";
                var SuMulti = $scope.POKEMONBATTLE.CALCS.MULTIPLIER(info.pokemonTarget.types, info.move.type) === "=-1";
            ...
            MyEffect: function (info, $scope, critial, miss, callback) {
                ...
                if (info.move.num === 367) {
                    ...
                    $scope.ACTIONS.ANIMATION.PLAYNATURAL(animation,
                        $scope.POKEMONBATTLE.XS(info.friend.body.x + (info.friend.body.width / 2)),
                        $scope.POKEMONBATTLE.YS(info.friend.body.y + (info.friend.body.height / 2)),
                        function () {
                            if (!twocallback)
                ...
                if (info.move.flags)
                    if (info.move.flags.heal) {
                        if (info.move.drain) {
                            var healme = $scope.POKEMONBATTLE.CALCS.DAMAGE($scope, info, critial, true, false);
            ...
            GIGA: function (info, $scope, pokemon, friend, callback) {
                ...
                if (pokemon.battle.gigamax) {
                    ...
                    $scope.ACTIONS.ANIMATION.THROWNATURAL("gigamaxdown",
                        $scope.POKEMONBATTLE.XS(friend.body.x + (friend.body.width / 2)),
                        $scope.POKEMONBATTLE.YS(friend.body.y + (friend.body.height / 2)),
                        $scope.POKEMONBATTLE.XS(friend.body.x + (friend.body.width / 2)),
                        $scope.POKEMONBATTLE.YS(friend.body.y + (friend.body.height / 2)),
                        true, function () {
                ...
                if (pokemon.giga) {
                    if (prob > 98) {
                        ...
                        $scope.ACTIONS.ANIMATION.THROWNATURAL("gigamax",
                            $scope.POKEMONBATTLE.XS(friend.body.x + (friend.body.width / 2)),
                            $scope.POKEMONBATTLE.YS(friend.body.y + (friend.body.height / 2)),
                            $scope.POKEMONBATTLE.XS(friend.body.x + (friend.body.width / 2)),
                            $scope.POKEMONBATTLE.YS(friend.body.y + (friend.body.height / 2)),
            ...
            TRANSFORM: function ($scope, info, callback) {
                ...
                $scope.ACTIONS.ANIMATION.THROWNATURAL("transform" + trasnum,
                    $scope.POKEMONBATTLE.XS(info.friend.body.x + (info.friend.body.width / 2)),
                    $scope.POKEMONBATTLE.YS(info.friend.body.y + (info.friend.body.height / 2)),
                    $scope.POKEMONBATTLE.XS(info.friend.body.x + (info.friend.body.width / 2)),
                    $scope.POKEMONBATTLE.YS(info.friend.body.y + (info.friend.body.height / 2)),
                    true, function () {
                        ...
                        if (info.isEnemy) {
                            ...
                            $scope.POKEMONBATTLE.RETARGET($scope);
                            $scope.ACTIONS.SOUND.PLAY($scope.PKM.friend().cryUrl, $scope.SOUNDS.system);
                        } else {
                            ...
                            $scope.POKEMONBATTLE.REFRIEND($scope);
                            $scope.ACTIONS.SOUND.PLAY($scope.PKM.target().cryUrl, $scope.SOUNDS.system);
                        }
                        if (callback)
                            callback();
            ...
            TRANSFORMDIRECT: function ($scope, friend, pokemonFriend, isEnemy) {
                $scope.POKEMONBATTLE.CALCS.TRANSFORM($scope, {
                    friend: friend,
        ...
        CALCANIMATION: function ($scope, result, info, callback, last) {
            $scope.POKEMONBATTLE.CALCS.GIGA(info, $scope, info.pokemonFriend, info.friend, function () {
                if (result)
                    if (result.miss) {
                        info.enemy.body.gotoAndPlay($scope.POKEMONBATTLE.FORMS.evade);
                        $scope.ACTIONS.ANIMATION.PLAYNATURAL("b_intrerrogation",
                            $scope.POKEMONBATTLE.XS(info.friend.body.x + (info.friend.body.width / 2)),
                            $scope.POKEMONBATTLE.YS(info.friend.body.y + (info.friend.body.height / 2)),
                            function () {
                                if (callback)
                ...
                if (result)
                    if (result.critical) {
                        info.hero.body.gotoAndPlay($scope.POKEMONBATTLE.FORMS.hurt);
                        $scope.ACTIONS.ANIMATION.PLAYNATURAL('critico', $scope.POKEMONBATTLE.XS(info.target.body.x + (info.target.body.width / 2)),
                            $scope.POKEMONBATTLE.YS(info.target.body.y + (info.target.body.height / 2)));
                    }
                if (callback)
                    callback(false);
...
//Turnos
        TURNING: function ($scope, info, callback, last) {
            ...
            if (info.move.keyname === "catchpokemon") {
                ...
                info.hero.body.gotoAndPlay($scope.POKEMONBATTLE.FORMS.punch);
                $scope.ACTIONS.ANIMATION.THROWNATURAL('Pokeball_' + ($scope.PKM.pokeballLevel + 1),
                    info.hero.body.x,
                    info.hero.body.y,
                    $scope.POKEMONBATTLE.XS(info.target.body.x + (info.target.body.width / 2)),
                    $scope.POKEMONBATTLE.YS(info.target.body.y + (info.target.body.height / 2)),
                    true,
                    function () {
                        $scope.ACTIONS.ANIMATION.PLAYNATURAL("GetBack",
                            $scope.POKEMONBATTLE.XS(info.target.body.x + (info.target.body.width / 2)),
                            $scope.POKEMONBATTLE.YS(info.target.body.y + (info.target.body.height / 2)),
                            function () {
                                $scope.ACTIONS.GAME.ALPHABASE(info.target.body, 0.3, 0);
                                $scope.ACTIONS.ANIMATION.PLAYNATURAL('Pokeball_' + ($scope.PKM.pokeballLevel + 1),
                                    $scope.POKEMONBATTLE.XS(info.target.body.x + (info.target.body.width / 2)),
                                    $scope.POKEMONBATTLE.YS(info.target.body.y + (info.target.body.height)),
                                    function () {
                                    }, "stop_pela");
                                                            ...
                                                            $timeout(() => {
                                                                    ...
                                                                    if (!catched) {
                                                                        ...
                                                                        $scope.ACTIONS.ANIMATION.PLAYNATURAL(info.pokemonTarget.superShiny ? 'SuperShiny' : (info.pokemonTarget.shiny ? 'Starts' : 'OutPokeball'),
                                                                            ...
                                                                            $scope.POKEMONBATTLE.YS(info.target.body.y + (info.target.body.height / 2)),
                                                                            function () {
                                                                                $scope.ACTIONS.GAME.ALPHABASE(info.target.body, 0.3, 1);
            ...
            for (var poke of info.team) {
                if (poke.ability.keyname === "pastelveil")
                    if (info.pokemonFriend.battle.status === "tox") {
                        $scope.ACTIONS.ANIMATION.PLAYNATURAL('Cureit', $scope.POKEMONBATTLE.XS(info.friend.body.x + (info.friend.body.width / 2)), $scope.POKEMONBATTLE.YS(info.friend.body.y + (info.friend.body.height / 2)));
                        info.pokemonFriend.battle.status = undefined;
                        info.pokemonFriend.battle.statusTurn = 0;
                    }
            ...
            if (info.move.keyname === "transform") {
                $scope.POKEMONBATTLE.CALCS.TRANSFORM($scope, info, function () {
            ...
            if (info.pokemonFriend.battle.status === "Wish") {
                ...
                info.pokemonFriend.battle.status = undefined;
                info.pokemonFriend.battle.statusTurn = 0;
                $scope.ACTIONS.ANIMATION.PLAYNATURAL("wishouts",
                    $scope.POKEMONBATTLE.XS(info.friend.body.x + (info.friend.body.width / 2)),
                    $scope.POKEMONBATTLE.YS(info.friend.body.y + (info.friend.body.height / 2)),
                    function () {
            ...
            if (["conversion2"].indexOf(info.move.keyname) !== -1) {
                if ($scope.PKM.lastMove) {
                    var newtype = "";
                    for (var t in $scope.APIS.TYPES[$scope.PKM.lastMove.type].damageTaken) {
                        if ($scope.APIS.TYPES[$scope.PKM.lastMove.type].damageTaken[t] === 2) {
                            newtype = t;
                            break;
                        }
                    }
            ...
            if ($scope.BATTLEOBJS.tourmentMove) {
                if ($scope.BATTLEOBJS.tourmentMove === info.move.num) {
                    ...
                    $scope.ACTIONS.ANIMATION.PLAYNATURAL("flinch",
                        $scope.POKEMONBATTLE.XS(info.friend.body.x + (info.friend.body.width / 2)),
                        $scope.POKEMONBATTLE.YS(info.friend.body.y + (info.friend.body.height / 2)),
                        function () {
            ...
            if ($scope.BATTLEOBJS.numTurn % 3 === 0) {
                if (["kingsshield", "protect", "detect", "matblock", "quickguard"].indexOf(info.pokemonTarget.battle.status) !== -1) {
                    ...
                    $scope.ACTIONS.ANIMATION.PLAYNATURAL("barrier",
                        $scope.POKEMONBATTLE.XS(info.target.body.x + (info.target.body.width / 2)),
                        $scope.POKEMONBATTLE.YS(info.target.body.y + (info.target.body.height / 2)),
                        function () {
            ...
            if (["metronome", "sleeptalk"].indexOf(info.move.keyname) !== -1) {
                info.move = OSO(randomObject($scope.APIS.MOVES));
            }
            ...
            if (info.move.forceSwitch) {
                if (info.isEnemy) {
                    ...
                    if (lives.length > 0) {
                        ...
                        $scope.POKEMONBATTLE.CHANGEPOKEMON($scope, $scope.BATTLEOBJS.FRIENDINDEX, function () {
                            if (callback)
                    ...
                    if ($scope.BATTLEOBJS.TARGETS.length > 1) {
                        ...
                        if (indexCambiar) {
                            $scope.POKEMONBATTLE.CHANGETARGET($scope, indexCambiar, false, function () {
                                if (callback)
                                    callback({volatile: "", critical: false, miss: false, damage: 0, last: last});
                            });
                        } else {
            ...
            if ($scope.PKM.hp(info.pokemonFriend) <= 0) {
                ...
                if (info.pokemonFriend.battle.status === "endure") {
                    ...
                    info.pokemonFriend.battle.status = undefined;
                    $scope.POKEMONBATTLE.RUNLOSE($scope);
                    if (callback)
                        callback({critical: false, miss: true, damage: 0});
                    return;
                }
            ...
            if (info.isEnemy) {
                ...
                if (info.move.num === 226) {
                    if ($scope.BATTLEOBJS.TARGETS.length > 1) {
                        ...
                        $scope.POKEMONBATTLE.CHANGETARGET($scope, 0, false, function () {
                                $timeout(() => {
                                    $scope.POKEMONBATTLE.RUNLOSE($scope);
                                    if (callback)
                                        callback(result);
            ...
            var miss = $scope.POKEMONBATTLE.CALCS.IsMISS($scope, info, last);
            ...
            var volatileStop = $scope.POKEMONBATTLE.CALCS.VolatileStatus(info, $scope, last, miss);
            var critical = $scope.POKEMONBATTLE.CALCS.IsCRITICAL($scope, info, miss);
            ...
            var damage = $scope.POKEMONBATTLE.CALCS.DAMAGE($scope, info, critical, false, last, miss);
            ...
            $scope.POKEMONBATTLE.CALCS.Status(result, info, $scope, function (statusstop, animation) {
                if (statusstop) {
                    ...
                    $timeout(() => {
                            if ($scope.PKM.hp(info.pokemonTarget) <= 0) {
                                ...
                                if (info.pokemonTarget.battle.status === "endure") {
                                    ...
                                    $scope.POKEMONBATTLE.CHANGETARGET($scope, $scope.BATTLEOBJS.TARGETINDEX, true);
                                }
                            }
                            $scope.POKEMONBATTLE.RUNLOSE($scope);
                    ...
                    if (info.move.sound) {
                        $scope.ACTIONS.SOUND.PLAY(info.pokemonFriend.cryUrl, $scope.SOUNDS.system);
                    }
                    $scope.POKEMONBATTLE.ATTACK($scope, info.move, info.hero, info.enemy, info.friend, info.target,
                        function () {
                            $scope.POKEMONBATTLE.CALCS.MyEffect(info, $scope, critical, miss, function () {

                                if (damage < 0) {
                                    $scope.ACTIONS.ANIMATION.PLAYNATURAL("b_strange",
                                        $scope.POKEMONBATTLE.XS(info.friend.body.x + (info.friend.body.width / 2)),
                                        $scope.POKEMONBATTLE.YS(info.friend.body.y + (info.friend.body.height / 2)),
                                        function () {
                                            if (callback)
        ...
        RUNTURN: function ($scope, move, change) {
            ...
            $timeout(() => {
                    var enemyAction = {
                        move: OSO($scope.PKM.target().moves [$scope.POKEMONBATTLE.IAMOVE($scope)]),
                        ...
                        emotion: $scope.POKEMONBATTLE.FORMS.point_bad,
                        isEnemy: true,
                    ...
                    if (change !== undefined) {
                        ...
                        $scope.POKEMONBATTLE.CHANGEPOKEMON($scope, change, function () {
                                ...
                                $timeout(() => {
                                    ...
                                    if (!whyisDead) {
                                        $scope.POKEMONBATTLE.TURNING($scope, enemyAction, function (result) {
                                            $scope.POKEMONBATTLE.CALCANIMATION($scope, result, enemyAction, function (stop) {
                                                $scope.PKM.mainMenu = true;
                                            }, true);
                                        }, true);
                                    } else {
                        ...
                        var enemyChange = $scope.POKEMONBATTLE.IACHANGE($scope);
                        if (enemyChange.change) {
                            $scope.POKEMONBATTLE.CHANGETARGET($scope, enemyChange.index, false, function () {
                                    ...
                                    $timeout(() => {
                                        ...
                                        $scope.POKEMONBATTLE.TURNING($scope, friendAction, function (result) {
                                            $scope.POKEMONBATTLE.CALCANIMATION($scope, result, friendAction, function (stop) {
                                                $scope.PKM.mainMenu = true;
                                            }, true);
                                        }, true);
                                    }, 1000);
                            ...
                            $scope.POKEMONBATTLE.TURNING($scope, primero, function (result) {
                                $scope.POKEMONBATTLE.CALCANIMATION($scope, result, primero, function (stop) {
                                    if (stop === false) {
                                        $scope.POKEMONBATTLE.TURNING($scope, segundo, function (result) {
                                            $scope.POKEMONBATTLE.CALCANIMATION($scope, result, segundo, function () {
                                                $scope.POKEMONBATTLE.PERSONALITY($scope);
                                                $scope.PKM.mainMenu = true;
                                            }, true);
                                        }, true);
                                    } else {
            ...
            if ($scope.BATTLEOBJS.trainerProp) {
                if (winOrLose === undefined) {
                    if ($scope.BATTLEOBJS.numTurn % 5 === 0) {
                        var mycount = $scope.POKEMONBATTLE.LIVES($scope, $scope.session.pokemons, 99).length;
                        ...
                        $scope.ACTIONS.MESSAGE.ADDPLAY($scope.BATTLEOBJS.trainerProp.name, $scope.personCheck(textFinal, move, pokemon), 4000);
                    }
                } else {
                    if (winOrLose === true) {
                        var textFinal = randomArray($scope.BATTLEOBJS.trainerProp.texts.alperder);
                        $scope.ACTIONS.MESSAGE.ADDPLAY($scope.BATTLEOBJS.trainerProp.name, $scope.personCheck(textFinal, move, pokemon), 5000);
                    } else {
...
//Inteligencia Artificial
        IAMOVE: function ($scope) {
            ...
            for (var move of moves) {
                ...
                var multiplier = $scope.POKEMONBATTLE.CALCS.MULTIPLIER(tyypes, move.type, true);
                if (multiplier < 1) {
                        ...
                        for (var des of supertengomaspoderosos) {
                            for (var mov of $scope.PKM.friend().moves) {
                                if (mov.category !== "Status") {
                                    ...
                                    var conataqueeste = $scope.POKEMONBATTLE.CALCS.MULTIPLIER(tyypes, mov.type, true);
                                    if (conataqueeste === 4) {
                                        descartado.push(des);
        ...
        IACHANGE: function ($scope) {
            ...
            if ($scope.BATTLEOBJS.TARGETS.length > 1) {
                ...
                for (var mov of $scope.PKM.target().moves) {
                    if (mov.category !== "Status") {
                        var conataqueeste = $scope.POKEMONBATTLE.CALCS.MULTIPLIER($scope.PKM.friend().types, mov.type, true);
                        if (conataqueeste === 4) {
                            conataqueesteint++;
                            ...
                            if (vanivel1 || vanivel2) {
                                for (var mover of $scope.PKM.friend().moves) {
                                    if (mover.category !== "Status") {
                                        var andaa = $scope.POKEMONBATTLE.CALCS.MULTIPLIER(mypok.types, mover.type, true);
                                        if (andaa === 4) {
                ...
                for (var mypok of $scope.BATTLEOBJS.TARGETS) {
                    ...
                    for (var mov of $scope.PKM.friend().moves) {
                        if (mov.category !== "Status") {
                            var conataqueeste = $scope.POKEMONBATTLE.CALCS.MULTIPLIER(mypok.types, mov.type, true);
                            var vanivel1 = false;
                            var vanivel2 = false;
                            if (conataqueeste === 2) {
                                vanivel1 = true;
                            ...
                            if (vanivel1 || vanivel2) {
                                for (var mover of $scope.PKM.friend().moves) {
                                    if (mover.category !== "Status") {
                                        var conataqueestecontrarestar = $scope.POKEMONBATTLE.CALCS.MULTIPLIER(mypok.types, mover.type, true);
                ...
                if (maspoderoso || muchomaspoderoso) {
                    ...
                    if (sguir) {
                        ...
                        //maspoderosos
                        var cambio = $scope.POKEMONBATTLE.IAMASPODEROSO($scope, muchomaspoderoso, supertengomaspoderosos, 100, propfrad);
                        if (cambio)
                            return cambio;
                        cambio = $scope.POKEMONBATTLE.IAMASPODEROSO($scope, maspoderoso, supertengomaspoderosos, 60, propfrad);
                        if (cambio)
                            return cambio;
                        cambio = $scope.POKEMONBATTLE.IAMASPODEROSO($scope, maspoderoso, tengomaspoderosos, 30, propfrad);
                        if (cambio)
                            return cambio;
                        //maspoderosos
                    }
                ...
                cambio = $scope.POKEMONBATTLE.IAMASANALYST($scope, medebilitaconmucho, lodebilitaconmucho, 100, propfrad);
                if (cambio) return cambio;
                cambio = $scope.POKEMONBATTLE.IAMASANALYST($scope, medebilitaconmucho, lodebilitacon, 95, propfrad);
                if (cambio) return cambio;
                cambio = $scope.POKEMONBATTLE.IAMASANALYST($scope, medebilitaconmucho, resistoconmucho, 85, propfrad);
                if (cambio) return cambio;
                cambio = $scope.POKEMONBATTLE.IAMASANALYST($scope, medebilitaconmucho, resistocon, 75, propfrad);
                if (cambio) return cambio;

                cambio = $scope.POKEMONBATTLE.IAMASANALYST($scope, medebilitacon, lodebilitaconmucho, 100, propfrad);
                if (cambio) return cambio;
                cambio = $scope.POKEMONBATTLE.IAMASANALYST($scope, medebilitacon, lodebilitacon, 95, propfrad);
                if (cambio) return cambio;
                cambio = $scope.POKEMONBATTLE.IAMASANALYST($scope, medebilitacon, resistoconmucho, 75, propfrad);
                if (cambio) return cambio;
                cambio = $scope.POKEMONBATTLE.IAMASANALYST($scope, medebilitacon, resistocon, 70, propfrad);
                if (cambio) return cambio;
...
//The End
        ...
        END: function ($scope, win) {
            ...
            for (var poke of $scope.session.pokemons) {
                $scope.POKEMONBATTLE.CLEARSTATSEND(poke);
            }
            if (win === "left") {
                $scope.BATTLEOBJS.HERO.body.gotoAndStop($scope.POKEMONBATTLE.FORMS.evade);
                $timeout(() => {
            ...
            if (win === true) {
                $scope.BATTLEOBJS.HERO.body.gotoAndStop($scope.POKEMONBATTLE.FORMS.happy);
                $scope.BATTLEOBJS.ENEMY.body.gotoAndStop($scope.POKEMONBATTLE.FORMS.dead);
                $scope.POKEMONBATTLE.PERSONALITY($scope, true);
                if ($scope.CURRENTONLINERATING !== undefined) {
                    $scope.session.rating = ($scope.session.rating || 0);
                    var myrating = $scope.session.rating;
                    if (myrating === $scope.CURRENTONLINERATING) {
                        $scope.session.rating++;
                    }
                    if ($scope.CURRENTONLINERATING > myrating) {
            ...
            if (win === "catched") {
                ...
                if (typeof $scope.BATTLEOBJS.winbattle === "function")
                    ...
                    eval($scope.BATTLEOBJS.winbattle);
                $scope.play("PokeCaught", $scope.SOUNDS.system);
                $timeout(() => {
                    $scope.POKEMONBATTLE.ENDFINAL($scope, $scope.PKM.target());
                }, 5000);
            }
            if (win === false) {
                $scope.BATTLEOBJS.ENEMY.body.gotoAndStop($scope.POKEMONBATTLE.FORMS.happy);
                $scope.BATTLEOBJS.HERO.body.gotoAndStop($scope.POKEMONBATTLE.FORMS.dead);
                $scope.POKEMONBATTLE.PERSONALITY($scope, false);
                if ($scope.CURRENTONLINERATING !== undefined) {
                    $scope.session.rating = $scope.session.rating || 0;
                    var myrating = $scope.session.rating;
                    if (myrating === $scope.CURRENTONLINERATING) {
                ...
                $scope.play("Defeat", $scope.SOUNDS.system);
                $timeout(() => {
                        $scope.POKEMONBATTLE.ENDFINAL($scope, false);
                    }, 7000
                )
            }
        },
        ENDFINAL: function ($scope, pokemon) {
            $scope.ACTIONS.GAME.SAVE(function () {
                $scope.ACTIONS.GAME.SCREEN(0.5, "#000", 1);
                $scope.PKM.mainMenu = false;
                ...
                $scope.ACTIONS.GAME.SCREENOFF(1, function () {
                    ...
                    if (pokemon === false) {
                        eval($scope.maps[$scope.FIRSTMAP].pokecenter);
                        if ($scope.maps[$scope.FIRSTMAP].pokecenter.indexOf('POKEMON.POKECENTER') === -1)
                            $scope.ACTIONS.POKEMON.POKECENTER();
                    } else if (pokemon) {
                        $scope.POKEMOMFIND.ADD(OSO(pokemon));
                    }
                    $scope.$evalAsync();
                });
...
//Changes
        CHANGEPOKEMON: function ($scope, index, callback, isdead) {
            var pass = "";
            if (isdead)
                $scope.POKEMONBATTLE.CLEARSTATSEND($scope.PKM.friend());
            else
            ...
            $scope.ACTIONS.GAME.ALPHABASE($scope.BATTLEOBJS.FRIEND.body, 0.2, 0);
            $scope.BATTLEOBJS.HERO.body.gotoAndPlay($scope.POKEMONBATTLE.FORMS.punch);
            var animationBack = $scope.BATTLEOBJS.batonpass ? "BatonAnimation" : "GetBack";
            $scope.BATTLEOBJS.batonpass = false;

            $scope.ACTIONS.ANIMATION.PLAYNATURAL(animationBack,
                $scope.POKEMONBATTLE.XS($scope.BATTLEOBJS.FRIEND.body.x + ($scope.BATTLEOBJS.FRIEND.body.width / 2)),
                $scope.POKEMONBATTLE.YS($scope.BATTLEOBJS.FRIEND.body.y + ($scope.BATTLEOBJS.FRIEND.body.height / 2)),
                function () {
        ...
        CHANGETARGET: function ($scope, index, isdead, callback) {
            ...
            if (isdead) {
                if ($scope.BATTLEOBJS.TARGETS.length === 1) {
                    $scope.POKEMONBATTLE.END($scope, true);
                    return;
                }
                $scope.BATTLEOBJS.TARGETS.splice($scope.BATTLEOBJS.TARGETINDEX, 1);
                $scope.BATTLEOBJS.TARGETINDEX = 0;
                index = 0;
                $scope.BATTLEOBJS.TARGETINDEX = $scope.POKEMONBATTLE.IACHANGE($scope);
            }
            $scope.PKM.pokemonDetail = false;
            $scope.PKM.menu = false;


            $scope.ACTIONS.GAME.ALPHABASE($scope.BATTLEOBJS.TARGET.body, 0.2, 0);
            $scope.BATTLEOBJS.ENEMY.body.gotoAndPlay($scope.POKEMONBATTLE.FORMS.punch);
            $scope.ACTIONS.ANIMATION.PLAYNATURAL("GetBack",
                $scope.POKEMONBATTLE.XS($scope.BATTLEOBJS.TARGET.body.x + ($scope.BATTLEOBJS.TARGET.body.width / 2)),
                $scope.POKEMONBATTLE.YS($scope.BATTLEOBJS.TARGET.body.y + ($scope.BATTLEOBJS.TARGET.body.height / 2)),
        ...
        ATTACK: function ($scope, cmove, hero, enemy, friend, target, callback, miss) {
            ...
            hero.body.gotoAndPlay($scope.POKEMONBATTLE.FORMS.punch);

            var move = $scope.APIS.MOVES.filter(d => {
            ...
            for (var step of hows) {
                ...
                eval(`
            $timeout(()=> {
                 $scope.POKEMONBATTLE.SUBANIMATE($scope, hero, enemy, friend, target, '${codes[0]}','${codes[1]}','${codes[2]}','${codes[3]}');
            }, ${time});`);
        ...
        SUBANIMATE: function ($scope, hero, enemy, friend, target, body, movement, animation, sufri, callback) {
            ...
            if (friend.isHide) {
                friend.isHid = false;
                $scope.ACTIONS.GAME.ALPHABASE(friend.body, 1, 1);
            }
            if (body === "up") {
                friend.isHide = true;
                $scope.ACTIONS.GAME.ALPHABASE(friend.body, 1, 0);
                $scope.ACTIONS.ANIMATION.DIRECT(friend.body, friend.body.x, -2000, 2000, function () {
            ...
            if (body === "throw") {
                $scope.ACTIONS.ANIMATION.DIRECT(friend.body, target.body.x, target.body.y, 500, function () {

                });
            }

            if (body === "self") {
                $scope.ACTIONS.GAME.SHAKEY(friend.body, 100, 30, 1);
            }

            var vsufri = sufri || "shake";
            var codesufri = "";
            if (vsufri === "shake") {
                codesufri = "$scope.ACTIONS.GAME.SHAKE(target.body, 20, 20, 5);";
            }
            if (vsufri === "supershake") {
                codesufri = "$scope.ACTIONS.GAME.SHAKE(target.body, 10, 30, 40);";
            }


            if (movement === "full") {
                eval(codesufri);
                $scope.ACTIONS.ANIMATION.THROWNATURAL(animation,
                    0, -100,
                    0, -100,
                    true, function () {
                        eval(codesufri);
            ...
            if (movement === "special") {
                $scope.ACTIONS.SOUND.PLAY(friend.body.cry, $scope.SOUNDS.system);
                $scope.ACTIONS.ANIMATION.THROWNATURAL(animation,
                    $scope.POKEMONBATTLE.XS(friend.body.x + (friend.body.width / 2)),
                    $scope.POKEMONBATTLE.YS(friend.body.y + (friend.body.height / 2)),
                    $scope.POKEMONBATTLE.XS(friend.body.x + (friend.body.width / 2)),
                    ...
                    true, function () {
                        $scope.ACTIONS.ANIMATION.THROWNATURAL(animation,
                            $scope.POKEMONBATTLE.XS(friend.body.x + (friend.body.width / 2)),
                            $scope.POKEMONBATTLE.YS(friend.body.y + (friend.body.height / 2)),
                            $scope.POKEMONBATTLE.XS(target.body.x + (target.body.width / 2)),
                            $scope.POKEMONBATTLE.YS(target.body.y + (target.body.height / 2)),
                            true, function () {
                                eval(codesufri);
                                $scope.ACTIONS.ANIMATION.THROWNATURAL(animation,
                                    $scope.POKEMONBATTLE.XS(target.body.x + (target.body.width / 2)),
            ...
            if (movement === "up") {
                $scope.ACTIONS.ANIMATION.THROWNATURAL(animation,
                    $scope.POKEMONBATTLE.XS(friend.body.x + (friend.body.width / 2)),
                    $scope.POKEMONBATTLE.YS(friend.body.y + (friend.body.height / 2)),
                    $scope.POKEMONBATTLE.XS(friend.body.x + (friend.body.width / 2)), -1000,
                    true, function () {
                    }, undefined, undefined, undefined, undefined, undefined);
            }
            if (movement === "down") {
...
//Draws
        DRAWBG: function ($scope) {
            ...
            var BG = new createjs.Bitmap($scope.mapQueues[$scope.FIRSTMAP].getResult("BG"), new createjs.Rectangle(0, 0, imageD.width, imageD.height));
            BG.scaleX = 0.575;
            BG.scaleY = 0.5;
        ...
        HERO: function ($scope) {
            ...
            var heroShadow = new createjs.Bitmap($scope.mapQueues["player_" + $scope.session.id].getResult(`SHADOWBIG`));
        ...
        FRIEND: function ($scope) {
            ...
            sprite.x = $scope.POKEMONBATTLE.X(($scope.BATTLEOBJS.HERO.body.x - wdif) + 50);
            var hdif = 70 - bound.height;
            sprite.y = $scope.POKEMONBATTLE.Y(($scope.BATTLEOBJS.HERO.body.y + 25) + (hdif));
            ...
            shadow.x = $scope.POKEMONBATTLE.X(($scope.BATTLEOBJS.HERO.body.x - wdif) + 50) - 20;
            shadow.y = $scope.POKEMONBATTLE.Y(($scope.BATTLEOBJS.HERO.body.y + 25) + (hdif) + bound.height);
...
//Tests
        FRIENDTEST: function ($scope, index) {
            ...
            $scope.ACTIONS.LOAD.ADD([prepokemon.imageUrl.replace('/front/', '/back/').replace('/front_s/', '/back_s/'), prepokemon.cryUrl], function () {
