...
POKEDEX_ = {
    ...
    Pokemon: (name, isShiny) => new Promise(async (resolve, reject) => {
        ...
        console.log(poke);
        var pokepath = DOMAINRESOURCE + `resources/poekemon/gif/front${shiny > 50 ? "_s" : ""}/${name}.gif`;
        var sound_pokepath = DOMAINRESOURCE + `resources/poekemon/audio/cries/${POKEDEX_.cleanName(poke.baseSpecies || name)}.ogg`;
        if (poke.num <= 721) {
            sound_pokepath = DOMAINRESOURCE + `resources/poekemon/audio/cries_anime/${POKEDEX_.cleanName(poke.baseSpecies || name)}.ogg`;
